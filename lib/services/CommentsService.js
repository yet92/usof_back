...
const {CommentNotFound, LikeNotFound} = require("../helpers/errors");
...
class CommentsService {
    ...
    #checkCommentVisibility(comment, {userRole, userId}) {
        ...
        if (comment.posts && comment.posts[0]?.author_id === userId) return true;
        return userRole === 'admin' ||
            comment.author?.id === Number(userId) ||
            comment.post?.author_id === Number(userId);
    }
