...
function useAuth({User, AuthToken, createEmailConfirmation, createPasswordReset, resetPassword, userMiddleware, createUser}) {
    ...
    router.post('/login', [validate(loginValidation)],
        async function (req, res, next) {
            ...
            try {
                ...
                if (!user) {
                    const error = {name: 'Login Error', message: 'User not found'};
                    return res.status(404).json(error);
                }

                if (!user.isEmailConfirmed) {
                    const error = {name: 'Login Error', message: 'Confirm email'};
                ...
                if (!passwordCheck) {
                    const error = {name: 'Login Error', message: 'Invalid password'};
                    return res.status(403).json(error);
                }
    ...
    router.post('/password-reset', [validate(passwordResetRequestValidation)],
        async function (req, res, next) {
    ...
    router.post('/password-reset/:token', [validate(passwordResetValidation)],
        async function (req, res, next) {
